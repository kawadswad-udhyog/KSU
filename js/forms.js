...
const FormEngine = {
    ...
    showFallbackNotice(containerEl, title, message) {
        ...
        waLink.className = 'inline-flex items-center justify-center gap-2 px-6 py-3 bg-[#FE330E] text-white text-xs font-bold uppercase tracking-wider rounded-xl hover:bg-[#d92500] transition-colors shadow-sm cta-whatsapp';
...
function initBusinessForm() {
    const form = document.querySelector('form[data-form="business"], #business-enquiry-form, #business-general-form, #careerForm');
    if (!form) return;

    const nameInput = form.querySelector('#gen-name, #contact-person, #fullName, input[name="name"]');
    const mobileInput = form.querySelector('#gen-mobile, #business-mobile, #phone, input[name="mobile"]');
    const emailInput = form.querySelector('#gen-email, #business-email, #email, input[name="email"]');
    const messageInput = form.querySelector('#gen-message, #business-message, #message, textarea[name="message"]');
